...
handle_sqlite() {
	...
	COL_MAXZOOM=$(python3 $MINTCAST_PATH/python/macro_mbtiles minzoom $MBTILES_FILEPATH)
	COL_MINZOOM=$(python3 $MINTCAST_PATH/python/macro_mbtiles maxzoom $MBTILES_FILEPATH)
	COL_VALUE_ARRAY=''
	COL_VECTOR_JSON=''
	if [[ $COL_RASTER_OR_VECTOR_TYPE = 'vector' ]]; then
	...
	if [[ "$HAS_LAYER" = "None" ]]; then
		python3 $MINTCAST_PATH/python/macro_sqlite_curd insert layer \
