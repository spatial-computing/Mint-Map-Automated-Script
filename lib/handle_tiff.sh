...
handle_tiff(){
    ...
    # Generate raster tiles:
        ...
        # if [[ ! -z "$START_TIME" ]]; then
            if [[ "$GENERATE_NEW_RES" == "YES" ]]; then
                # with new res proc
                MAX_VAL=$(python3 $MINTCAST_PATH/python/macro_gdal max-value $INPUT $NETCDF_SINGLE_SUBDATASET)
                MIN_VAL=$(python3 $MINTCAST_PATH/python/macro_gdal min-value $INPUT $NETCDF_SINGLE_SUBDATASET)
